...
const MAX_CHARS = 64;                                   // 角色索引上界（客户端 ROSTER 目前 21 名）
...
export const PROTO_VER = 1;                             // 联机协议版本，客户端必须匹配
...
let quickQueue = null;                 // 快速匹配等待者
const hostedRooms = new Map();         // 房号 -> 等待中的房主连接
let roomSeq = 0;
...
function sendStart(room) {
    ...
    /* P3-6：开局水位重置为 0（原 -1 会让首帧取到 MAX_ABS_FRAME 内任意值，一次性顶穿滑动窗口）。
       帧号计数器与单调去重游标同步复位，再战时帧号从 0 重新递增。 */
    room.maxF = 0;
    for (const p of room.peers) { p.lastInF = -1; p.lastHashF = -1; }
    room.peers.forEach(p => send(p, start));
    console.log(`[room ${room.id}] 开战 seed=${start.seed} delay=${start.delay}`);
}

/* 无效消息记账：超过阈值断开；附带审计日志 */
function bad(ws, kind, err) {
    ws.badCount++;
    if (ws.badCount <= 3) console.warn(`[ws] 无效消息(${kind}) bad=${ws.badCount} addr=${addrOf(ws)}${err ? ' err=' + err.message : ''}`);
    if (ws.badCount >= MAX_BAD) {
        console.warn(`[ws] 无效消息过多，断开 ${addrOf(ws)}`);
...
function handleMessage(ws, msg) {
    ...
    if (!ws.room) {
        if (msg.t === 'quick') {
            if (quickQueue && quickQueue !== ws && quickQueue.readyState === 1) {
                const peer = quickQueue; quickQueue = null;
                pair(peer, ws);
            } else {
                quickQueue = ws;
                send(ws, { t: 'wait' });
                console.log('[lobby] 快速匹配等待中');
            }
            ...
            if (ws.hostCode) hostedRooms.delete(ws.hostCode);   // 回收旧房号
            const code = genCode();
            if (!code) {
                ws.hostCode = null;
            ...
            ws.lastCreateAt = now;
            ws.hostCode = code;
            hostedRooms.set(code, ws);
            send(ws, { t: 'room', code });
            console.log(`[lobby] 创建房间 ${code}（当前等待房间 ${hostedRooms.size}）`);
        } else if (msg.t === 'join') {
            const code = msg.code;
            if (typeof code !== 'string' || !ROOM_CODE_RE.test(code)) { bad(ws, 'join-code'); return; }
            const host = hostedRooms.get(code);
            ...
            if (!host || host.readyState !== 1) {
                ...
                if (ws.joinFails > MAX_JOIN_FAILS) {
                    console.warn(`[audit] join 连续失败过多，断开 ${addrOf(ws)}`);
                    try { ws.terminate(); } catch { /* ignore */ }
                }
                return;
            }
    ...
    if (msg.t === 'sel') {
        ...
        room.picks[ws.side] = { c: msg.c, wins, roundTime };
        ...
        room.peers.forEach(p => { p.room = null; p.side = -1; });
        console.log(`[room ${room.id}] 对局中止: ${reason}`);
...
export function createNetServer({ port = PORT, host = HOST, maxConns = MAX_CONNS, allowedOrigins = null, heartbeatMs = HEARTBEAT_MS } = {}) {
    ...
    server.on('error', err => console.error('[net] 服务器错误:', err.message));

    server.on('connection', (ws, req) => {
        /* 连接配额（P3-5：原为 `> maxConns`，实际放行 maxConns+1 个连接） */
        if (maxConns > 0 && server.clients.size >= maxConns) {
            console.warn(`[audit] 连接数达上限 ${maxConns}，拒绝 ${addrOf(ws)}`);
        ...
        ws.on('error', err => {
            console.error(`[ws] 连接错误 ${addrOf(ws)}:`, err.message);
        });

        ws.on('close', () => {
            if (quickQueue === ws) quickQueue = null;
            if (ws.hostCode) hostedRooms.delete(ws.hostCode);
            const room = ws.room;
            if (room && !room.closed) {
                room.closed = true;
                const peer = room.peers[1 - ws.side];
                if (peer && peer.readyState === 1) { peer.room = null; peer.side = -1; send(peer, { t: 'peerLeft' }); }
                console.log(`[room ${room.id}] 玩家离开，房间解散`);
            }
            ws.room = null;
            ws.side = -1;
            console.log(`[audit] 连接关闭 addr=${addrOf(ws)}`);
        });
    });
    ...
    const heartbeat = setInterval(() => {
        for (const ws of server.clients) {
            if (ws.isAlive === false) {
                console.warn(`[ws] 心跳超时，断开 ${addrOf(ws)}`);
                try { ws.terminate(); } catch { /* ignore */ }
                continue;
            }
            ws.isAlive = false;
...
if (isMain) {
    ...
    server.wss.on('listening', () => {
        console.log(`[net] 联机服务器已启动 ws://${HOST || '0.0.0.0'}:${server.wss.address().port}`
